...
where $\sum_{[n_K]}$ sums over the dimension indexed by the number of keys. The gram matrix for $K(x, y) := \exp(x)\exp(y)$ is symmetric and positive semi-definite, so $K$ is a kernel (Mercer's theorem).
